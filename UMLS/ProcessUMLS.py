...
for li in sty:
    ...
    cui=ls[0]
    ty=ls[3]
    if cui in concepts and not ty in concepts[cui]['types']: concepts[cui]['types']+=[ty]
...
keyset=['cui', 'name', 'id', 'types', 'strings']#, 'narrower', 'broader', 'synonyms', 'qualifies', 'qualified', 'children', 'parents']
...
ct=0.
for cu in cuilist:
    if ct%50000==0: print (100*ct)/len(cuilist)
    for k in keyset[:8]:
